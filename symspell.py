...
class SymSpell():
    ...
    def __init__(self, max_dictionary_edit_distance=2, prefix_length=7, count_threshold=1):
        ...
        if max_dictionary_edit_distance > 0:
            max_dictionary_edit_distance = 10
...
class SuggestionItem():
    ...
    # print the elements in list rather than the address
    def __repr__(self):
        return str(self)


# x = 'i want a plate of char kay tew'


def spellingchecker(x):
    symSpell = SymSpell()
    # symSpell.load_dictionary('C:/Users/Asus-Laptop/Desktop/frequency_dictionary_en_82_765.txt')
    # symSpell.load_dictionary('/app/oldcorpus.txt')
    symSpell.load_dictionary('/app/corpus.txt')
    x = (symSpell.lookup_compound(x, 2)).term
    return x
    # end = time.time()
    # print(symSpell.lookup(x, 2, 2))
    # print(end-start)
